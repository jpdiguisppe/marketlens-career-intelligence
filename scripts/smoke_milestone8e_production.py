...
def object_json(status: int, text: str, *, context: str, expected: int = 200) -> dict[str, Any]:
...
def frontend_source_contract(root: Path) -> dict[str, Any]:
    ...
    require(not missing, "Frontend source contract is missing markers: " + ", ".join(missing))
    ...
    return {
        "status": "passed",
        "marker_count": len(app_markers) + len(telemetry_markers),
...
def frontend_bundle_contract() -> dict[str, Any]:
    ...
    status, config = request_text(f"{FRONTEND_URL}/config.js?milestone8e_canary={cache_buster}")
    require(status == 200, f"Frontend runtime config returned HTTP {status}")
    match = re.search(r"apiBaseUrl\s*:\s*[\"']([^\"']+)[\"']", config)
    require(match is not None, "Frontend runtime config does not expose apiBaseUrl")
    configured_api = match.group(1).rstrip("/")
...
def wait_for_deployment() -> dict[str, Any]:
    ...
    for attempt in range(1, DEPLOYMENT_ATTEMPTS + 1):
        try:
            ...
            return {"attempt": attempt, "health": health, "frontend": frontend_bundle_contract()}
        except CanaryFailure as exc:
            failures.append(f"attempt {attempt}: {exc}")
            if attempt < DEPLOYMENT_ATTEMPTS:
                time.sleep(DEPLOYMENT_RETRY_SECONDS)
    raise CanaryFailure("Production did not reach the expected deployment state: " + " | ".join(failures[-5:]))
...
def validate_stage(stage: Any, *, requested: bool) -> dict[str, Any]:
    ...
    require(stage["outcome"] in {"not_requested", "used", "unavailable", "fallback"}, "Unknown provider outcome")
    require(isinstance(stage["status_code"], str) and len(stage["status_code"]) <= 96, "Unsafe provider status code")
    require(isinstance(stage["latency_ms"], (int, float)) and stage["latency_ms"] >= 0, "Invalid provider latency")
    usage = stage["usage"]
    if usage is not None:
        ...
        for field in ("input_tokens", "cached_input_tokens", "output_tokens", "reasoning_tokens", "total_tokens"):
            require(isinstance(usage.get(field), int) and usage[field] >= 0, f"Invalid token field: {field}")
    cost = stage["estimated_cost_usd"]
    require(cost is None or (isinstance(cost, (int, float)) and cost >= 0), "Invalid stage cost")
    return {
        "stage": stage["stage"],
...
def validate_telemetry(payload: dict[str, Any], *, requested: bool) -> dict[str, Any]:
    ...
    require(isinstance(telemetry["total_tokens"], int) and telemetry["total_tokens"] >= 0, "Invalid total token count")
    serialized = json.dumps(telemetry, sort_keys=True)
    require(CANARY_RESUME not in serialized and CANARY_JOB not in serialized, "Provider telemetry contains request documents")
    return {
...
def validate_smart_fit(payload: dict[str, Any], *, requested: bool) -> dict[str, Any]:
    ...
    require(payload["analysis_engine"] in {"deterministic", "model_assisted"}, "Unknown analysis engine")
    require(isinstance(payload["model_assisted_status"], str) and payload["model_assisted_status"], "Missing model status")
    require(isinstance(payload["report_summary"], list) and payload["report_summary"], "Smart Fit report summary is empty")
    require(isinstance(payload["category_coverage"], list) and payload["category_coverage"], "Category coverage is empty")
    fit = payload["fit_summary"]
    require(isinstance(fit, dict), "Fit summary is not an object")
    require(isinstance(fit.get("score"), (int, float)) and 0 <= fit["score"] <= 100, "Fit score is out of range")
    require(isinstance(fit.get("confidence"), (int, float)) and 0 <= fit["confidence"] <= 1, "Confidence is out of range")
    assessments = payload["requirement_assessments"]
    require(isinstance(assessments, list), "Requirement assessments are not a list")
    require(all(item.get("grounded") is True for item in assessments), "Smart Fit returned ungrounded conclusions")
    return {
...
def model_status() -> dict[str, Any]:
    ...
    require(payload["status"] in {"configured", "not_configured"}, "Unknown model configuration status")
    require(payload["enabled"] is (payload["status"] == "configured"), "Model enabled/status fields disagree")
    return {
...
def safe_validation_error() -> dict[str, Any]:
    ...
    require(payload.get("detail") == "Request validation failed.", "HTTP 422 response did not use the safe detail")
    errors = payload.get("errors")
    require(isinstance(errors, list) and errors, "HTTP 422 response is missing bounded error metadata")
    require(all(set(error).issubset({"type", "location"}) for error in errors), "HTTP 422 response exposed extra fields")
    return {"status": status, "detail": payload["detail"], "error_count": len(errors), "rejected_value_echoed": False}


def production_canary(root: Path) -> dict[str, Any]:
    deterministic = run_smart_fit(assisted=False)
    require(deterministic["analysis_engine"] == "deterministic", "Non-requested Smart Fit was not deterministic")
    require(
    ...
    if fell_back:
        require(assisted["model_assisted_status"].startswith("fallback"), "Fallback status was not explicit")
        require(
            assisted["deterministic_fingerprint"] == deterministic["deterministic_fingerprint"],
            "Provider fallback did not preserve the deterministic report fingerprint",
        )
    else:
        require(assisted["model_assisted_status"] == "used", "Model-assisted result did not expose used status")
        require(assisted["fit_score"] == deterministic["fit_score"], "Model-assisted path changed the deterministic score")

    return {
        "canary_version": "8e.5.1",
...
if __name__ == "__main__":
    try:
        ...
        print(f"MILESTONE 8E PRODUCTION CANARY FAILED: {exc}", file=sys.stderr)
